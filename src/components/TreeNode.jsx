...
export default function TreeNode({ node, isRoot = false }) {
  ...
  const styles = {
    director: {
      card: `bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 text-white border-slate-600 `,
      container: "bg-slate-50 border-slate-200",
      icon: "👔",
    },

    subordinate: {
      card: `bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-700 text-white border-emerald-500`,
      container: "bg-emerald-50 border-emerald-200",
      icon: "🧑‍💼",
    },

    member: {
      card: `bg-gradient-to-br from-white to-slate-50 text-slate-800 border-slate-200`,
      container: "bg-slate-50 border-slate-200",
      icon: "👤",
    },
  };
  ...
  `}
      >
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="text-2xl px-2 font-bold hover:bg-white/20 rounded-md cursor-pointer transition">
              ⋮
            </button>
          </div>
        </div>
        ...
        {showMenu && (
          <div className="mt-4 flex flex-col gap-2 animate-[fadeInScale_0.25s_ease-out]">
            {node.type === "director" && (
              <button
                onClick={() => {
            ...
            {node.type === "member" && (
              <button
                onClick={() => {
                  dispatch(addSubBranchMember(node.id));
                  setShowMenu(false);
                }}
                className="bg-white text-orange-700 rounded-lg px-4 py-2 font-medium hover:bg-orange-50 transition cursor-pointer">
